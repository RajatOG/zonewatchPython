        ...
        if os.path.isdir(session_path):
            ...
            if current_time - os.path.getmtime(session_path) > max_age:
                try:
                    ...
                    logging.info(f"Removed old session directory: {session_dir}")
                except Exception as e:
                    logging.error(f"Error removing session directory {session_dir}: {str(e)}")
    ...
    for session_dir in os.listdir(RESULTS_FOLDER):
        ...
        if os.path.isdir(session_path):
            ...
            if current_time - os.path.getmtime(session_path) > max_age:
                try:
                    shutil.rmtree(session_path)
                    logging.info(f"Removed old results directory: {session_dir}")
                except Exception as e:
                    logging.error(f"Error removing results directory {session_dir}: {str(e)}")
